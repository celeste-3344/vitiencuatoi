...
/* =====================================================
   NÉN + CẮT VUÔNG ẢNH ĐẠI DIỆN (avatar)
   Khác với ảnh minh chứng: avatar luôn hiển thị trong khung
   tròn nên ta crop vuông ở giữa ảnh trước, resize nhỏ (tối đa
   360px) rồi nén JPEG ~82% để giữ nét mặt rõ mà vẫn nhẹ.
===================================================== */
...
function compressAvatarImage(file){
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      ...
      img.onload = () => {
        try {
          // Cắt vuông ở giữa ảnh gốc (theo cạnh ngắn hơn).
          const side = Math.min(img.width, img.height);
          const sx = (img.width - side) / 2;
          const sy = (img.height - side) / 2;
          const size = Math.min(AVATAR_MAX_SIZE, side);
...
// Dựng nội dung 1 khung avatar: ảnh thật (nếu có photoURL) hoặc chữ cái đầu tên.
function renderAvatarInto(el, name, photoURL){
  if(!el) return;
  const initial = (name || '').trim().charAt(0).toUpperCase() || '?';
  if(photoURL){
    el.innerHTML = `<img src="${photoURL}" alt="Ảnh đại diện">`;
  } else {
...
function renderUser(){
  ...
  renderAvatarInto(document.getElementById('userAvatar'), appData.user.name, appData.user.photoURL);
  renderAvatarInto(document.getElementById('userAvatarMobile'), appData.user.name, appData.user.photoURL);
...
function openProfileModal(){
  ...
  renderAvatarInto(document.getElementById('profileAvatarPreview'), appData.user.name, appData.user.photoURL);
  document.getElementById('btnRemoveAvatar').classList.toggle('hidden', !appData.user.photoURL);
...
// Chọn ảnh mới cho avatar: nén + crop vuông, lưu ngay lên Firestore và cập nhật giao diện tức thì.
async function handleAvatarFileSelect(event){
  ...
  try {
    ...
    await withLoading(() => Data.saveProfile({ photoURL: dataUrl }), 'Đang cập nhật ảnh đại diện...');
    renderAvatarInto(document.getElementById('profileAvatarPreview'), appData.user.name, appData.user.photoURL);
    document.getElementById('btnRemoveAvatar').classList.remove('hidden');
    renderUser();
...
async function removeAvatar(){
  if(!appData.user.photoURL) return;
  await withLoading(() => Data.saveProfile({ photoURL: '' }), 'Đang xóa ảnh đại diện...');
  renderAvatarInto(document.getElementById('profileAvatarPreview'), appData.user.name, appData.user.photoURL);
  document.getElementById('btnRemoveAvatar').classList.add('hidden');
  renderUser();
...
async function handleSaveProfile(e){
  ...
  try {
    ...
    if(err.message === 'USERNAME_TAKEN'){
      showToast('Tên đăng nhập này đã có người khác dùng. Vui lòng chọn tên khác.', 'error');
    } else {
      console.error(err);
...
function switchView(view){
  ...
  ['dashboard','transactions','budgets','savings','wallets','debts','analytics','recurring','notifications','settings','group'].forEach(v => {
  ...
  const titles = {
    ...
    savings:      ['Tích lũy', 'Đặt mục tiêu và theo dõi tiến độ để dành của bạn'],
    ...
    group:        ['Không gian nhóm', 'Ví và giao dịch dùng chung với các thành viên trong nhóm'],
  };
  ...
  if(view === 'group') renderGroupSpace();
      ...
      <tr>
        ...
        <td>
          <div class="tx-actions">
            <button class="tx-del" onclick="openModal('${t.id}')" title="Sửa giao dịch"><i class="fa-solid fa-pen"></i></button>
            <button class="tx-del" onclick="deleteTransaction('${t.id}')" title="Xóa giao dịch"><i class="fa-solid fa-trash"></i></button>
          </div>
        </td>
      </tr>
    `;
...
function renderDonutChart(){
  ...
  const chartWrapEl = document.getElementById('donutChartWrap');
  if(entries.length === 0){
    holder.innerHTML = '';
    if(chartWrapEl) chartWrapEl.style.display = 'none'; // không chừa chỗ trống cho vòng tròn khi chưa có dữ liệu
    legend.innerHTML = `<div class="empty-state">Chưa có chi tiêu nào được ghi nhận trong tháng này.</div>`;
    return;
  }
  ...
  legend.innerHTML = entries.map(([id, val]) => {
    ...
    return `<div class="legend-row">
      ...
      <span class="legend-amounts"><span class="amt">${formatCurrency(val)}</span><span class="pct">${pct}%</span></span>
    </div>`;
  }).join('');
}

// monthKey ("YYYY-MM") của tháng liền trước một monthKey cho trước.
function prevMonthKeyOf(monthKey){
  const [y, m] = monthKey.split('-').map(Number);
  return getMonthKeyOffset(new Date(y, m - 1, 1), -1);
}

/* =====================================================
   NGÂN SÁCH CỘNG DỒN (ROLLOVER BUDGET)
   Nếu tháng trước tiêu chưa hết hạn mức, phần dư (chỉ tính khi
   dư, không tính khi bội chi) sẽ được cộng thêm vào hạn mức
   hiển thị của tháng đang xem — với điều kiện ngân sách này đã
   tồn tại trước tháng đang xem (ngân sách mới tạo trong tháng
   hiện tại thì chưa có "tháng trước" nào để cộng dồn).
===================================================== */
function computeBudgetRollover(b, monthKey){
  if(b.rollover === false) return 0;
  const [y, m] = monthKey.split('-').map(Number);
  const monthStart = new Date(y, m - 1, 1).getTime();
  if(b.createdAt && b.createdAt >= monthStart) return 0;

  const prevKey = prevMonthKeyOf(monthKey);
  const prevSpent = appData.transactions
    .filter(t => t.type === 'expense' && t.category === b.category && t.date.startsWith(prevKey))
    .reduce((s,t) => s + t.amount, 0);
  return Math.max(0, b.limit - prevSpent);
}
function renderBudgetOverview(monthKey){
  ...
  const rows = appData.budgets.map(b => {
    ...
    const spent = appData.transactions
      ...
      .reduce((s,t) => s + t.amount, 0);
    const rollover = computeBudgetRollover(b, monthKey);
    const effectiveLimit = b.limit + rollover;
    const pct = Math.min(100, Math.round((spent / effectiveLimit) * 100));
    return { cat, limit:effectiveLimit, spent, pct, isOver: spent > effectiveLimit * 0.8 };
  });
...
function renderBudgets(targetId, monthKey){
  ...
  list.innerHTML = appData.budgets.map(b => {
    ...
    const effectiveLimit = b.limit + rollover;
    const pct = Math.min(100, Math.round((spent / effectiveLimit) * 100));
    const isOver = spent > effectiveLimit * 0.8;
        ...
        <div class="budget-top">
          ...
          <div class="budget-right">
            <div class="budget-numbers">
              <span class="num-line"><strong>${formatCurrency(spent)}</strong> / ${formatCurrency(effectiveLimit)}</span>
              ${rollover > 0 ? `<span class="budget-rollover-tag" title="Hạn mức gốc ${formatCurrency(b.limit)} + dư tháng trước ${formatCurrency(rollover)}"><i class="fa-solid fa-arrow-trend-up"></i> +${formatCurrency(rollover)} dư tháng trước</span>` : ''}
            </div>
            <div class="budget-row-edit">
              <button class="icon-btn ${b.rollover !== false ? 'active' : ''}" title="${b.rollover !== false ? 'Đang cộng dồn số dư tháng trước — bấm để tắt' : 'Đang tắt cộng dồn — bấm để bật lại'}" onclick="toggleBudgetRollover('${b.category}')"><i class="fa-solid fa-arrow-rotate-right"></i></button>
...
function openBudgetModal(categoryId){
  ...
  const rolloverInput = document.getElementById('budgetInputRollover');
  ...
  if(isEdit){
    ...
    rolloverInput.checked = existing ? existing.rollover !== false : true;
  } else {
    limitInput.value = '';
    rolloverInput.checked = true;
  }
...
async function handleSaveBudget(e){
  ...
  const rollover = document.getElementById('budgetInputRollover').checked;
  ...
  await withLoading(() => Data.saveBudget(category, limit, rollover), 'Đang lưu ngân sách...');
...
// Bật/tắt nhanh cộng dồn ngân sách ngay từ danh sách, không cần mở modal Sửa ngân sách.
async function toggleBudgetRollover(category){
  const b = appData.budgets.find(x => x.category === category);
  if(!b) return;
  const wasOn = b.rollover !== false;
  await withLoading(() => Data.saveBudget(category, b.limit, !wasOn), 'Đang cập nhật...');
  renderCore();
  showToast(wasOn ? 'Đã tắt cộng dồn ngân sách cho danh mục này.' : 'Đã bật cộng dồn ngân sách cho danh mục này.');
}

document.getElementById('budgetModalOverlay').addEventListener('click', (e) => { if(e.target.id === 'budgetModalOverlay') closeBudgetModal(); });
document.getElementById('savingsGoalModalOverlay').addEventListener('click', (e) => { if(e.target.id === 'savingsGoalModalOverlay') closeSavingsGoalModal(); });
document.getElementById('savingsDepositModalOverlay').addEventListener('click', (e) => { if(e.target.id === 'savingsDepositModalOverlay') closeSavingsDepositModal(); });
document.getElementById('groupCreateModalOverlay').addEventListener('click', (e) => { if(e.target.id === 'groupCreateModalOverlay') closeGroupCreateModal(); });
document.getElementById('groupInviteModalOverlay').addEventListener('click', (e) => { if(e.target.id === 'groupInviteModalOverlay') closeGroupInviteModal(); });
document.getElementById('groupWalletModalOverlay').addEventListener('click', (e) => { if(e.target.id === 'groupWalletModalOverlay') closeGroupWalletModal(); });
document.getElementById('groupTxModalOverlay').addEventListener('click', (e) => { if(e.target.id === 'groupTxModalOverlay') closeGroupTransactionModal(); });
document.getElementById('groupMemberQrModalOverlay').addEventListener('click', (e) => { if(e.target.id === 'groupMemberQrModalOverlay') closeGroupMemberQrModal(); });
...
// Đồng bộ ô mã HEX khi người dùng chọn màu qua color picker — dùng chung cho
// mọi modal có cặp color-picker + ô hex (Danh mục, Mục tiêu tích lũy...).
...
function renderWallets(){
  ...
  const savingsWalletSel = document.getElementById('savingsDepositWallet');
  if(savingsWalletSel){
    const prevSavingsWallet = savingsWalletSel.value;
    savingsWalletSel.innerHTML = opts;
    if(appData.wallets.some(w => w.id === prevSavingsWallet)) savingsWalletSel.value = prevSavingsWallet;
  }
}
...
/* =====================================================
   RENDER + MODAL: MỤC TIÊU TÍCH LŨY (SAVINGS GOALS)
===================================================== */
function renderSavingsGoals(){
  ...
  if(appData.savingsGoals.length === 0){
    grid.innerHTML = `<div class="panel"><div class="empty-state">Chưa có mục tiêu tích lũy nào. Nhấn "Thêm mục tiêu" để bắt đầu để dành cho một dự định nào đó.</div></div>`;
    return;
  }

  grid.innerHTML = appData.savingsGoals.map(g => {
    const pct = g.targetAmount > 0 ? Math.min(100, Math.round((g.currentAmount / g.targetAmount) * 100)) : 0;
    const isDone = g.currentAmount >= g.targetAmount && g.targetAmount > 0;
    return `
      <div class="savings-card">
        <div class="sg-top">
          <div class="sg-icon" style="background:${g.color}22;color:${g.color}"><i class="fa-solid ${g.icon}"></i></div>
          <div class="sg-actions">
            <button class="icon-btn" title="Sửa mục tiêu" onclick="openSavingsGoalModal('${g.id}')"><i class="fa-solid fa-pen"></i></button>
            <button class="icon-btn danger" title="Xóa mục tiêu" onclick="deleteSavingsGoal('${g.id}')"><i class="fa-solid fa-trash"></i></button>
          </div>
        </div>
        ...
        <div class="sg-numbers">
          <span class="sg-current">${formatCurrency(g.currentAmount)}</span>
          <span class="sg-target">/ ${formatCurrency(g.targetAmount)}</span>
        </div>
        <div class="sg-pct" style="color:${isDone ? 'var(--color-income)' : 'var(--text-muted)'}">${isDone ? 'Đã hoàn thành mục tiêu! 🎉' : `Đã đạt ${pct}%`}</div>
        <button type="button" class="btn btn-outline full" onclick="openSavingsDepositModal('${g.id}')"><i class="fa-solid fa-piggy-bank"></i> Nạp tiền vào quỹ</button>
      </div>
    `;
  }).join('');
}
...
function openSavingsGoalModal(id){
  document.getElementById('savingsGoalEditingId').value = id || '';
  document.getElementById('savingsGoalModalTitle').textContent = id ? 'Sửa mục tiêu tích lũy' : 'Thêm mục tiêu tích lũy';

  const g = id ? appData.savingsGoals.find(x => x.id === id) : null;
  document.getElementById('savingsGoalInputName').value = g ? g.name : '';
  document.getElementById('savingsGoalInputTarget').value = g ? g.targetAmount : '';
  const color = g ? g.color : '#5FB0B7';
  document.getElementById('savingsGoalInputColor').value = color;
  document.getElementById('savingsGoalInputColorHex').value = color.replace('#','').toUpperCase();
  document.getElementById('savingsGoalInputColorHex').classList.remove('invalid');
  renderIconPicker('savingsGoalIconPicker', 'savingsGoalInputIcon', g ? g.icon : 'fa-piggy-bank');

  document.getElementById('savingsGoalModalOverlay').classList.remove('hidden');
}
function closeSavingsGoalModal(){ document.getElementById('savingsGoalModalOverlay').classList.add('hidden'); }



async function handleSaveSavingsGoal(e){
  e.preventDefault();
  const id = document.getElementById('savingsGoalEditingId').value || null;
  const name = document.getElementById('savingsGoalInputName').value.trim();
  const targetAmount = parseFloat(document.getElementById('savingsGoalInputTarget').value);
  const color = document.getElementById('savingsGoalInputColor').value;
  const icon = document.getElementById('savingsGoalInputIcon').value;
  ...
  await withLoading(() => Data.saveSavingsGoal(id, { name, targetAmount, color, icon }), 'Đang lưu mục tiêu...');
  closeSavingsGoalModal();
  renderSavingsGoals();
  showToast(id ? 'Đã cập nhật mục tiêu tích lũy.' : `Đã tạo mục tiêu "${name}".`);
}

async function deleteSavingsGoal(id){
  const g = appData.savingsGoals.find(x => x.id === id);
  if(!g) return;
  if(!confirm(`Xóa mục tiêu "${g.name}"? Số tiền đã tích lũy trong quỹ này sẽ không được hoàn lại vào ví nào.`)) return;
  await withLoading(() => Data.removeSavingsGoal(id), 'Đang xóa mục tiêu...');
  renderSavingsGoals();
  showToast('Đã xóa mục tiêu tích lũy.');
}

function toggleSavingsDepositWalletField(useWallet){
  document.getElementById('savingsDepositWalletField').classList.toggle('hidden', !useWallet);
  document.getElementById('savingsDepositHint').textContent = useWallet
    ? 'Số dư ví đã chọn sẽ giảm tương ứng, y như một khoản chuyển tiền vào quỹ.'
    : 'Đây chỉ là con số theo dõi riêng — số dư ví và lịch sử giao dịch của bạn sẽ không thay đổi.';
}

function openSavingsDepositModal(id){
  const g = appData.savingsGoals.find(x => x.id === id);
  if(!g) return;
  document.getElementById('savingsDepositGoalId').value = id;
  document.getElementById('savingsDepositGoalName').textContent = g.name;
  document.getElementById('savingsDepositAmount').value = '';
  document.getElementById('savingsDepositUseWallet').checked = true;
  toggleSavingsDepositWalletField(true);
  document.getElementById('savingsDepositModalOverlay').classList.remove('hidden');
}
function closeSavingsDepositModal(){ document.getElementById('savingsDepositModalOverlay').classList.add('hidden'); }

async function handleDepositSavingsGoal(e){
  e.preventDefault();
  const goalId = document.getElementById('savingsDepositGoalId').value;
  const amount = parseFloat(document.getElementById('savingsDepositAmount').value);
  const useWallet = document.getElementById('savingsDepositUseWallet').checked;
  const walletId = useWallet ? document.getElementById('savingsDepositWallet').value : null;

  if(!amount || amount <= 0){ showToast('Vui lòng nhập số tiền hợp lệ.', 'error'); return; }
  if(useWallet && !walletId){ showToast('Vui lòng chọn ví để trừ tiền.', 'error'); return; }
  if(useWallet){
    const wallet = appData.wallets.find(w => w.id === walletId);
    if(wallet && wallet.balance < amount){
      if(!confirm(`Số dư ví "${wallet.name}" hiện không đủ (${formatCurrency(wallet.balance)}). Vẫn muốn tiếp tục (ví sẽ bị âm)?`)) return;
    }
  }

  await withLoading(() => Data.depositToSavingsGoal(goalId, amount, walletId), 'Đang nạp tiền vào quỹ...');
  closeSavingsDepositModal();
  renderSavingsGoals();
  renderWallets();
  renderWalletLog();
  showToast(useWallet ? 'Đã nạp tiền vào quỹ và trừ số dư ví tương ứng.' : 'Đã nạp tiền vào quỹ (không ảnh hưởng số dư ví).');
}
...
// bất kể nguồn dữ liệu là Ví, một khoản Công nợ, hay mã QR nhận tiền của 1 thành viên trong nhóm.
function getBankQrSource(){
  ...
  if(currentBankQrContext.type === 'groupMember'){
    const g = currentGroup();
    const info = g && g.memberBankInfo ? g.memberBankInfo[currentBankQrContext.id] : null;
    if(!info) return null;
    return { bankBin: info.bankBin, bankName: info.bankName, accountNumber: info.accountNumber, accountHolder: info.accountHolder, label: memberLabel(currentBankQrContext.id) };
  }
...
// type: 'wallet' | 'debt' | 'groupMember'
function openBankQrModal(type, id){
  currentBankQrContext = { type, id };
  const source = getBankQrSource();
  if(!source || !source.bankBin || source.bankBin === 'other' || !source.accountNumber){
    showToast(
      type === 'groupMember'
        ? 'Thành viên này chưa thiết lập mã QR nhận tiền.'
        : 'Chưa có đủ thông tin tài khoản ngân hàng (cần chọn đúng ngân hàng trong danh sách).',
      'error'
    );
    return;
  }
  ...
  // Với khoản công nợ, gợi ý sẵn số tiền còn lại và nội dung nhắc khoản nợ; với ví/QR thành viên thì để trống cho người dùng tự nhập.
  if(type === 'debt'){
...
initBankCombo({ comboId:'groupMemberBankCombo', searchId:'groupMemberBankBinSearch', hiddenId:'groupMemberBankBin', menuId:'groupMemberBankComboMenu' });
...
/* =====================================================
   DỰ BÁO DÒNG TIỀN (CASHFLOW PROJECTION)
   Ước tính chi tiêu tháng tới = trung bình chi tiêu 3 tháng gần nhất
   (đã tự nhiên bao gồm các khoản định kỳ lặp lại đều đặn hàng tháng).
   Tổng các khoản "Định kỳ" đang sắp đến hạn được tính riêng để hiển thị
   thêm trong chú thích — giúp thấy phần nào của tháng tới đã "có hẹn" sẵn.
===================================================== */
function computeCashflowProjection(){
  const now = new Date();
  const last3MonthKeys = [0,1,2].map(offset => getMonthKeyOffset(now, -offset));
  const monthlyExpenses = last3MonthKeys.map(mk =>
    appData.transactions.filter(t => t.type === 'expense' && t.date.startsWith(mk)).reduce((s,t) => s + t.amount, 0)
  );
  const avgExpense = monthlyExpenses.reduce((a,b) => a + b, 0) / monthlyExpenses.length;
  const recurringDueSum = appData.recurring.filter(r => r.status === 'due').reduce((s,r) => s + r.amount, 0);
  return { projectedExpense: Math.round(avgExpense), recurringDueSum };
}
...
function renderTrendChart(){
  ...
  // Chỉ áp dụng đường dự báo (nét đứt) cho chế độ "Năm nay", nối từ tháng hiện tại sang tháng kế tiếp
  // (nếu tháng hiện tại không phải tháng 12 — vì biểu đồ năm chỉ hiển thị cố định Tháng 1 -> Tháng 12).
  let projectionIndex = null, projectionInfo = null;
  if(currentAnalyticsRange === 'year'){
    const nowIdx = new Date().getMonth(); // 0 = Tháng 1
    if(nowIdx < 11){
      projectionInfo = computeCashflowProjection();
      projectionIndex = nowIdx + 1;
      expenseData[projectionIndex] = projectionInfo.projectedExpense;
    }
  }
  ...
  // Vẽ series chi tiêu có kèm đoạn dự báo (nét đứt) nối từ tháng hiện tại sang tháng kế tiếp.
  // Đường thật (solid) chỉ vẽ tới tháng hiện tại; các tháng chưa tới sau điểm dự báo để trống (chưa có dữ liệu).
  const expenseSeriesWithProjectionSvg = (data, color, name, projIdx) => {
    const realIdx = projIdx - 1;
    const realData = data.slice(0, realIdx + 1);
    const dashedPath = `M ${xFor(realIdx).toFixed(1)} ${yFor(data[realIdx]).toFixed(1)} L ${xFor(projIdx).toFixed(1)} ${yFor(data[projIdx]).toFixed(1)}`;
    return `
      <path d="${areaFor(realData)}" fill="${color}" fill-opacity="0.13" stroke="none"></path>
      <path d="${pathFor(realData)}" fill="none" stroke="${color}" stroke-width="2.5" stroke-linejoin="round" stroke-linecap="round"></path>
      <path d="${dashedPath}" fill="none" stroke="${color}" stroke-width="2.5" stroke-dasharray="6,5" stroke-linecap="round"></path>
      ${realData.map((v,i) => `<circle class="trend-pt" cx="${xFor(i).toFixed(1)}" cy="${yFor(v).toFixed(1)}" r="4" fill="${color}" stroke="var(--bg-card)" stroke-width="2"><title>${name} — ${labels[i]}: ${formatCurrency(v)}</title></circle>`).join('')}
      <circle class="trend-pt" cx="${xFor(projIdx).toFixed(1)}" cy="${yFor(data[projIdx]).toFixed(1)}" r="4.5" fill="var(--bg-card)" stroke="${color}" stroke-width="2.5"><title>Dự báo ${name.toLowerCase()} ${labels[projIdx]}: ${formatCurrency(data[projIdx])} (ước tính theo tốc độ chi tiêu hiện tại${projectionInfo && projectionInfo.recurringDueSum > 0 ? `, trong đó có ${formatCurrency(projectionInfo.recurringDueSum)} từ các khoản định kỳ sắp đến hạn` : ''})</title></circle>
    `;
  };
  ...
  holder.innerHTML = `
    <svg viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid meet">
      ...
      ${projectionIndex !== null
        ? expenseSeriesWithProjectionSvg(expenseData, '#D96868', 'Chi tiêu', projectionIndex)
    ...
    <div class="svg-legend">
      ...
      ${projectionIndex !== null ? `<span><span class="dot dot-dashed" style="border-color:#D96868"></span>Dự báo tháng tới</span>` : ''}
    </div>`;
}
...
function renderTopSpend(){
  ...
  if(sorted.length === 0){
    list.innerHTML = `<div class="empty-state">Chưa có chi tiêu trong tháng.</div>`;
    return;
  }
...
function openModal(id){
  editingTxId = id || null;
  const tx = editingTxId ? appData.transactions.find(t => t.id === editingTxId) : null;

  setTxType(tx ? tx.type : 'expense');
  document.getElementById('inputAmount').value = tx ? tx.amount : '';
  document.getElementById('inputNote').value = tx ? (tx.note || '') : '';
  document.getElementById('inputDate').value = tx ? tx.date : new Date().toISOString().slice(0,10);
  renderWallets();
  if(tx){
    document.getElementById('inputWallet').value = tx.walletId;
    document.getElementById('inputCategory').value = tx.category;
  }

  currentAttachments = tx ? [...(tx.attachments || [])] : [];
  ...
  document.getElementById('txSubmitBtn').innerHTML = `<i class="fa-solid fa-check"></i> ${tx ? 'Lưu thay đổi' : 'Lưu giao dịch'}`;
...
async function handleAddTransaction(e){
  ...
  if(editingTxId){
    // Sửa giao dịch: Data.updateTransaction tự tính lại đúng số dư ví
    // (kể cả khi đổi số tiền, loại thu/chi, hoặc đổi sang ví khác).
    await withLoading(() => Data.updateTransaction(editingTxId, payload), 'Đang lưu thay đổi...');
    closeModal();
    renderCore();
    showToast('Đã cập nhật giao dịch.');
  } else {
    // Firestore: ghi giao dịch + cập nhật số dư ví trong cùng một batch.
    await withLoading(() => Data.addTransaction(payload), 'Đang lưu giao dịch...');
    closeModal();
...
===================================================== */
let currentGroupTxType = 'expense';
let editingGroupWalletId = null;
let editingGroupTxId = null;
let groupTxAttachments = []; // ảnh minh chứng (base64) đang đính kèm ở modal giao dịch nhóm

function resetGroupTxAttachments(){
  groupTxAttachments = [];
  const fileInput = document.getElementById('groupTxAttachInput');
  if(fileInput) fileInput.value = '';
  renderGroupTxAttachPreview();
}

function renderGroupTxAttachPreview(){
  const grid = document.getElementById('groupTxAttachPreviewGrid');
  const desc = document.getElementById('groupTxAttachDropDesc');
  if(!grid) return;
  grid.innerHTML = groupTxAttachments.map((src, idx) => `
    <div class="attach-thumb">
      <img src="${src}" onclick="previewGroupTxAttachmentAt(${idx})" alt="Ảnh minh chứng ${idx+1}">
      <button type="button" class="rm-btn" title="Xóa ảnh" onclick="removeGroupTxAttachment(${idx})"><i class="fa-solid fa-xmark"></i></button>
    </div>`).join('');
  if(desc){
    desc.textContent = groupTxAttachments.length >= MAX_ATTACHMENTS
      ? `Đã đạt tối đa ${MAX_ATTACHMENTS} ảnh`
      : `Nhấn để chọn ảnh hoá đơn / biên lai (${groupTxAttachments.length}/${MAX_ATTACHMENTS})`;
  }
}

function handleGroupTxAttachSelect(e){
  const files = Array.from(e.target.files || []);
  if(files.length === 0) return;

  const remaining = MAX_ATTACHMENTS - groupTxAttachments.length;
  if(remaining <= 0){
    showToast(`Bạn chỉ có thể đính kèm tối đa ${MAX_ATTACHMENTS} ảnh.`, 'error');
    e.target.value = '';
    return;
  ...
  filesToUse.forEach(file => {
    ...
    compressImage(file)
      .then(dataUrl => {
        groupTxAttachments.push(dataUrl);
        renderGroupTxAttachPreview();
      })
      .catch(() => { showToast('Không thể đọc một trong các ảnh đã chọn.', 'error'); });
  });
...
function removeGroupTxAttachment(idx){
  groupTxAttachments.splice(idx, 1);
  renderGroupTxAttachPreview();
}

function previewGroupTxAttachmentAt(idx){
  lightboxImages = groupTxAttachments;
  lightboxIndex = idx;
  showLightbox();
}

function spaceSwitcherItems(){
  return [
    { value: 'personal', icon: 'fa-user', label: 'Không gian cá nhân' },
    ...appData.groups.map(g => ({ value: g.id, icon: 'fa-people-group', label: g.name })),
    { value: '__new_group__', icon: 'fa-plus', label: 'Tạo nhóm mới...', isNew: true },
  ];
}

function renderSpaceSwitcher(){
  const currentEl = document.getElementById('spaceSwitcherCurrent');
  const menu = document.getElementById('spaceSwitcherMenu');
  if(!currentEl || !menu) return;

  const current = appData.activeGroupId || 'personal';
  const items = spaceSwitcherItems();
  const activeItem = items.find(i => i.value === current) || items[0];

  currentEl.innerHTML = `<i class="fa-solid ${activeItem.icon}"></i> ${activeItem.label}`;
  menu.innerHTML = items.map(i => `
    <div class="space-switcher-item ${i.value === current ? 'active' : ''} ${i.isNew ? 'is-new' : ''}" onclick="selectSpaceSwitcherItem('${i.value}')">
      <i class="fa-solid ${i.icon}"></i> ${i.label}
    </div>
  `).join('');
}

function toggleSpaceSwitcherMenu(){
  document.getElementById('spaceSwitcherMenu').classList.toggle('hidden');
}
function closeSpaceSwitcherMenu(){
  document.getElementById('spaceSwitcherMenu').classList.add('hidden');
}

function selectSpaceSwitcherItem(value){
  closeSpaceSwitcherMenu();
  handleSpaceSwitch(value);
}

document.addEventListener('click', (e) => {
  const wrap = document.getElementById('spaceSwitcherWrap');
  if(wrap && !wrap.contains(e.target)) closeSpaceSwitcherMenu();
});

async function handleSpaceSwitch(value){
  if(value === '__new_group__'){
    openGroupCreateModal();
    return;
  }
  if(value === 'personal'){
    await switchToPersonalSpace();
  } else {
    await switchToGroupSpace(value);
  }
}

async function switchToPersonalSpace(){
  Data.exitGroupSpace();
  document.getElementById('personalNavItems').classList.remove('hidden');
  document.getElementById('groupNavItems').classList.add('hidden');
  renderSpaceSwitcher();
  switchView('dashboard');
}

async function switchToGroupSpace(groupId){
  await withLoading(() => Data.loadGroupSpace(groupId), 'Đang tải không gian nhóm...');
  document.getElementById('personalNavItems').classList.add('hidden');
  document.getElementById('groupNavItems').classList.remove('hidden');
  renderSpaceSwitcher();
  switchView('group');
}

function currentGroup(){
  return appData.groups.find(g => g.id === appData.activeGroupId) || null;
}

function memberLabel(uid){
  if(uid === appData.uid) return `${appData.user.name} (bạn)`;
  const g = currentGroup();
  if(!g) return 'Thành viên';
  const idx = g.members.indexOf(uid);
  const email = idx >= 0 ? g.memberEmails[idx] : '';
...
function renderGroupSpace(){
  ...
  document.getElementById('groupNameTitle').textContent = `${g.members.length} thành viên trong nhóm`;
  document.getElementById('pageTitle').textContent = g.name;

  document.getElementById('groupMembersRow').innerHTML = g.members.map((uid, i) => {
    const isMe = uid === appData.uid;
    const hasQr = g.memberBankInfo && g.memberBankInfo[uid] && g.memberBankInfo[uid].bankBin;
    return `
    <div class="member-chip" title="${isMe ? appData.user.email : (g.memberEmails[i] || '')}">
      <span class="member-chip-avatar">${memberLabel(uid).charAt(0).toUpperCase()}</span>
      ${memberLabel(uid)}
      <button type="button" class="member-chip-qr-btn" title="${isMe ? 'Thiết lập mã QR nhận tiền của tôi' : (hasQr ? 'Xem mã QR nhận tiền' : 'Thành viên này chưa có mã QR')}"
        onclick="${isMe ? 'openGroupMemberQrModal()' : `openBankQrModal('groupMember','${uid}')`}">
  ...
  const walletGrid = document.getElementById('groupWalletGrid');
  if(appData.groupWallets.length === 0){
    walletGrid.innerHTML = `<div class="panel"><div class="empty-state">Nhóm chưa có ví nào. Nhấn "Thêm ví nhóm" để bắt đầu chi tiêu chung.</div></div>`;
  } else {
    walletGrid.innerHTML = appData.groupWallets.map(w => `
      <div class="wallet-card" style="background:linear-gradient(135deg,#5FB0B7,#4C8A90);">
        <div class="w-top">
          <div class="w-top-left">
            <div class="w-icon"><i class="fa-solid ${w.icon || 'fa-people-group'}"></i></div>
            <div class="w-name">${w.name}</div>
          </div>
          <div class="sg-actions">
            <button class="icon-btn" style="color:#fff;" title="Sửa" onclick="openGroupWalletModal('${w.id}')"><i class="fa-solid fa-pen"></i></button>
            <button class="icon-btn" style="color:#fff;" title="Xóa" onclick="deleteGroupWallet('${w.id}')"><i class="fa-solid fa-trash"></i></button>
          </div>
        </div>
        <div class="w-balance">${formatCurrency(w.balance)}</div>
      </div>
  ...
  const tbody = document.getElementById('groupTxTableBody');
  if(appData.groupTransactions.length === 0){
    tbody.innerHTML = `<tr><td colspan="5" style="text-align:center;color:var(--text-muted);padding:24px 0;">Nhóm chưa có giao dịch nào.</td></tr>`;
  } else {
    tbody.innerHTML = appData.groupTransactions.map(t => {
      const cat = findCat(t.category);
      const sign = t.type === 'income' ? '+' : '−';
      const colorClass = t.type === 'income' ? 'income' : 'expense';
      return `
        <tr>
          <td class="tx-name-cell"><span class="tx-cat-icon" style="background:${cat.color}22;color:${cat.color}"><i class="fa-solid ${cat.icon}"></i></span> <div>${cat.label}${t.note ? ` — ${t.note}` : ''}${t.attachments && t.attachments.length ? ` <button type="button" class="tx-attach-badge" title="Xem ảnh minh chứng" onclick="openGroupTransactionModal('${t.id}')"><i class="fa-solid fa-paperclip"></i> ${t.attachments.length}</button>` : ''}</div></td>
          <td>${memberLabel(t.paidBy)}</td>
          <td class="tx-date">${formatDate(t.date)}</td>
          <td class="tx-amount ${colorClass}">${sign}${formatCurrency(t.amount)}</td>
          <td>
            <div class="tx-actions">
              <button class="icon-btn" title="Sửa giao dịch" onclick="openGroupTransactionModal('${t.id}')"><i class="fa-solid fa-pen"></i></button>
              <button class="icon-btn danger" title="Xóa" onclick="deleteGroupTransaction('${t.id}')"><i class="fa-solid fa-trash"></i></button>
            </div>
          </td>
        </tr>
      `;
...
function openGroupCreateModal(){
  document.getElementById('groupCreateInputName').value = '';
  document.getElementById('groupCreateModalOverlay').classList.remove('hidden');
}
function closeGroupCreateModal(){ document.getElementById('groupCreateModalOverlay').classList.add('hidden'); }

async function handleCreateGroup(e){
  e.preventDefault();
  const name = document.getElementById('groupCreateInputName').value.trim();
  if(!name){ showToast('Vui lòng nhập tên nhóm.', 'error'); return; }

  const groupId = await withLoading(() => Data.createGroup(name), 'Đang tạo nhóm...');
  closeGroupCreateModal();
  showToast(`Đã tạo nhóm "${name}".`);
  await switchToGroupSpace(groupId);
}

/* -------- Mời thành viên (email hoặc username) -------- */
function openGroupInviteModal(){
  document.getElementById('groupInviteInputIdentifier').value = '';
  document.getElementById('groupInviteModalOverlay').classList.remove('hidden');
}
function closeGroupInviteModal(){ document.getElementById('groupInviteModalOverlay').classList.add('hidden'); }

async function handleInviteGroupMember(e){
  e.preventDefault();
  const identifier = document.getElementById('groupInviteInputIdentifier').value.trim();
  if(!identifier) return;

  try {
    await withLoading(() => Data.addGroupMember(appData.activeGroupId, identifier), 'Đang thêm thành viên...');
    closeGroupInviteModal();
    renderGroupSpace();
    showToast(`Đã thêm ${identifier} vào nhóm.`);
  } catch(err){
    if(err.message === 'NOT_FOUND'){
      showToast('Không tìm thấy người dùng nào với email/username này. Họ cần đăng nhập ứng dụng ít nhất 1 lần trước.', 'error');
    } else if(err.message === 'ALREADY_MEMBER'){
      showToast('Người này đã ở trong nhóm rồi.', 'error');
    } else {
      console.error(err);
      showToast('Không thể thêm thành viên. Vui lòng thử lại.', 'error');
    }
  }
}

/* -------- Mã QR nhận tiền của tôi (trong 1 nhóm cụ thể) -------- */
function openGroupMemberQrModal(){
  const g = currentGroup();
  const info = g && g.memberBankInfo ? g.memberBankInfo[appData.uid] : null;
  document.getElementById('groupMemberBankBinSearch').value = info ? ((VIETQR_BANKS.find(b => b.bin === info.bankBin) || {}).name || info.bankName || '') : '';
  document.getElementById('groupMemberBankBin').value = info ? (info.bankBin || '') : '';
  document.getElementById('groupMemberAccountNumber').value = info ? (info.accountNumber || '') : '';
  document.getElementById('groupMemberAccountHolder').value = info ? (info.accountHolder || '') : '';
  document.getElementById('groupMemberQrModalOverlay').classList.remove('hidden');
}
function closeGroupMemberQrModal(){ document.getElementById('groupMemberQrModalOverlay').classList.add('hidden'); }

async function handleSaveGroupMemberQr(e){
  e.preventDefault();
  const bankBin = document.getElementById('groupMemberBankBin').value;
  const accountNumber = document.getElementById('groupMemberAccountNumber').value.trim();
  const accountHolder = document.getElementById('groupMemberAccountHolder').value.trim().toUpperCase();

  if(!bankBin){ showToast('Vui lòng chọn ngân hàng.', 'error'); return; }
  if(!accountNumber){ showToast('Vui lòng nhập số tài khoản.', 'error'); return; }
  if(!accountHolder){ showToast('Vui lòng nhập tên chủ tài khoản.', 'error'); return; }

  const bankName = (VIETQR_BANKS.find(b => b.bin === bankBin) || {}).name || '';
  await withLoading(() => Data.setMyGroupBankInfo(appData.activeGroupId, { bankBin, bankName, accountNumber, accountHolder }), 'Đang lưu mã QR...');
  closeGroupMemberQrModal();
  renderGroupSpace();
  showToast('Đã lưu mã QR nhận tiền của bạn cho nhóm này.');
}

/* -------- Ví nhóm -------- */
function openGroupWalletModal(id){
  editingGroupWalletId = id || null;
  const w = id ? appData.groupWallets.find(x => x.id === id) : null;
  document.getElementById('groupWalletInputName').value = w ? w.name : '';
  document.getElementById('groupWalletInputBalance').value = w ? w.balance : 0;
  document.getElementById('groupWalletModalOverlay').classList.remove('hidden');
}
function closeGroupWalletModal(){ document.getElementById('groupWalletModalOverlay').classList.add('hidden'); }

async function handleSaveGroupWallet(e){
  e.preventDefault();
  const name = document.getElementById('groupWalletInputName').value.trim();
  const balance = parseFloat(document.getElementById('groupWalletInputBalance').value) || 0;
  if(!name){ showToast('Vui lòng nhập tên ví.', 'error'); return; }

  await withLoading(() => Data.saveGroupWallet(appData.activeGroupId, editingGroupWalletId, { name, balance, icon: 'fa-people-group', gradient: '' }), 'Đang lưu ví nhóm...');
  closeGroupWalletModal();
  renderGroupSpace();
  showToast(editingGroupWalletId ? 'Đã cập nhật ví nhóm.' : 'Đã tạo ví nhóm mới.');
}

async function deleteGroupWallet(id){
  if(!confirm('Xóa ví nhóm này? Các giao dịch đã ghi nhận trong ví sẽ vẫn được giữ lại trong lịch sử.')) return;
  await withLoading(() => Data.removeGroupWallet(appData.activeGroupId, id), 'Đang xóa ví nhóm...');
  renderGroupSpace();
  showToast('Đã xóa ví nhóm.');
}

/* -------- Giao dịch nhóm (kèm gợi ý Chia tiền) -------- */
function setGroupTxType(type){
  currentGroupTxType = type;
  document.getElementById('groupTxBtnExpense').classList.toggle('active', type === 'expense');
  document.getElementById('groupTxBtnIncome').classList.toggle('active', type === 'income');
  const sel = document.getElementById('groupTxInputCategory');
  sel.innerHTML = CATS[type].map(c => `<option value="${c.id}">${c.label}</option>`).join('');
  renderGroupSplitSuggestion();
}

function renderGroupSplitSuggestion(){
  const box = document.getElementById('groupSplitSuggestion');
  const g = currentGroup();
  const amount = parseFloat(document.getElementById('groupTxInputAmount').value);
  if(!g || currentGroupTxType !== 'expense' || !amount || amount <= 0 || g.members.length < 2){
  ...
  box.innerHTML = `<i class="fa-solid fa-users-rectangle"></i> Chia đều cho ${g.members.length} thành viên: mỗi người khoảng <strong>${formatCurrency(perPerson)}</strong>.`;
}

function openGroupTransactionModal(id){
  editingGroupTxId = id || null;
  const tx = editingGroupTxId ? appData.groupTransactions.find(t => t.id === editingGroupTxId) : null;

  const walletSel = document.getElementById('groupTxInputWallet');
  const paidBySel = document.getElementById('groupTxInputPaidBy');

  walletSel.innerHTML = appData.groupWallets.map(w => `<option value="${w.id}">${w.name} (${formatCurrency(w.balance)})</option>`).join('');
  const g = currentGroup();
  paidBySel.innerHTML = g ? g.members.map(uid => `<option value="${uid}" ${(tx ? tx.paidBy === uid : uid === appData.uid) ? 'selected' : ''}>${memberLabel(uid)}</option>`).join('') : '';

  document.getElementById('groupTxInputAmount').value = tx ? tx.amount : '';
  document.getElementById('groupTxInputDate').value = tx ? tx.date : new Date().toISOString().slice(0,10);
  document.getElementById('groupTxInputNote').value = tx ? (tx.note || '') : '';
  setGroupTxType(tx ? tx.type : 'expense');
  if(tx){
    document.getElementById('groupTxInputCategory').value = tx.category;
    walletSel.value = tx.walletId;
  }
  document.getElementById('groupSplitSuggestion').classList.add('hidden');

  groupTxAttachments = tx ? [...(tx.attachments || [])] : [];
  const attachFileInput = document.getElementById('groupTxAttachInput');
  if(attachFileInput) attachFileInput.value = '';
  renderGroupTxAttachPreview();

  document.getElementById('groupTxModalTitle').textContent = tx ? 'Sửa giao dịch nhóm' : 'Thêm giao dịch nhóm';
  document.getElementById('groupTxSubmitBtn').innerHTML = `<i class="fa-solid fa-check"></i> ${tx ? 'Lưu thay đổi' : 'Lưu giao dịch'}`;

  if(appData.groupWallets.length === 0){
    showToast('Nhóm cần có ít nhất 1 ví trước khi thêm giao dịch.', 'error');
    return;
  }
  document.getElementById('groupTxModalOverlay').classList.remove('hidden');
}
function closeGroupTransactionModal(){
  document.getElementById('groupTxModalOverlay').classList.add('hidden');
  editingGroupTxId = null;
  resetGroupTxAttachments();
}

async function handleSaveGroupTransaction(e){
  e.preventDefault();
  const amount = parseFloat(document.getElementById('groupTxInputAmount').value);
  const category = document.getElementById('groupTxInputCategory').value;
  const date = document.getElementById('groupTxInputDate').value;
  const walletId = document.getElementById('groupTxInputWallet').value;
  const paidBy = document.getElementById('groupTxInputPaidBy').value;
  const note = document.getElementById('groupTxInputNote').value.trim();

  if(!amount || amount <= 0){ showToast('Vui lòng nhập số tiền hợp lệ.', 'error'); return; }
  if(!walletId){ showToast('Vui lòng chọn ví thanh toán.', 'error'); return; }

  const payload = { type: currentGroupTxType, category, amount, date, walletId, paidBy, note, attachments: [...groupTxAttachments] };

  if(editingGroupTxId){
    await withLoading(() => Data.updateGroupTransaction(appData.activeGroupId, editingGroupTxId, payload), 'Đang lưu thay đổi...');
    closeGroupTransactionModal();
    renderGroupSpace();
    showToast('Đã cập nhật giao dịch nhóm.');
  } else {
    await withLoading(() => Data.saveGroupTransaction(appData.activeGroupId, payload), 'Đang lưu giao dịch nhóm...');
    closeGroupTransactionModal();
    renderGroupSpace();
    showToast('Đã lưu giao dịch nhóm.');
  }
}

async function deleteGroupTransaction(id){
  if(!confirm('Xóa giao dịch này khỏi nhóm?')) return;
  await withLoading(() => Data.removeGroupTransaction(appData.activeGroupId, id), 'Đang xóa giao dịch...');
  renderGroupSpace();
  showToast('Đã xóa giao dịch nhóm.');
}
...
Object.assign(window, {
  ...
  handleAvatarFileSelect, removeAvatar,
  // Bảo mật / Mật khẩu
  ...
  // Ngân sách
  openBudgetModal, closeBudgetModal, handleSaveBudget, deleteBudget, toggleBudgetRollover,
  openSavingsGoalModal, closeSavingsGoalModal, handleSaveSavingsGoal, deleteSavingsGoal,
  syncColorFromPicker, syncColorFromHex,
  openSavingsDepositModal, closeSavingsDepositModal, toggleSavingsDepositWalletField, handleDepositSavingsGoal,
  toggleSpaceSwitcherMenu, selectSpaceSwitcherItem,
  handleSpaceSwitch, openGroupCreateModal, closeGroupCreateModal, handleCreateGroup,
  openGroupInviteModal, closeGroupInviteModal, handleInviteGroupMember,
  openGroupWalletModal, closeGroupWalletModal, handleSaveGroupWallet, deleteGroupWallet,
  setGroupTxType, renderGroupSplitSuggestion, openGroupTransactionModal, closeGroupTransactionModal,
  handleGroupTxAttachSelect, removeGroupTxAttachment, previewGroupTxAttachmentAt,
  openGroupMemberQrModal, closeGroupMemberQrModal, handleSaveGroupMemberQr,
  handleSaveGroupTransaction, deleteGroupTransaction,
  // Danh mục
