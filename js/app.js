...
function renderSpaceSwitcher(){
  const sel = document.getElementById('spaceSwitcherSelect');
  if(!sel) return;
  const current = appData.activeGroupId || 'personal';
  sel.innerHTML = `
    <option value="personal">🧍 Không gian cá nhân</option>
    ${appData.groups.map(g => `<option value="${g.id}">👥 ${g.name}</option>`).join('')}
    <option value="__new_group__">➕ Tạo nhóm mới...</option>
  `;
  sel.value = current;
}
...
async function handleSpaceSwitch(value){
  if(value === '__new_group__'){
    renderSpaceSwitcher(); // trả select về đúng lựa chọn hiện tại (không đổi thật)
...
function renderGroupSpace(){
  ...
  document.getElementById('groupNameTitle').textContent = g.name;
